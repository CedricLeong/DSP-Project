...
%is between 40-55ms. To do this we will sample the signal at 10kHz and if 
...
function [Signal] = RectifyingFilter() 
    ...
    figure;
    Sig1 = filter(b75,a75,fft(Signal));
    plot(abs(Sig1));
    ...
    figure;
    Sig2 = filter(b100,a100,fft(Signal));
    plot(abs(Sig2));
 ...
 %    % figure; 
 %    figure;
 %    Sig1 = fft(filter(b400,a400,Signal));
 %    plot(abs(Sig1));
